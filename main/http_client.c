...
const char *boundary = "x19randomstring23x";
...
static char *packhead(char *name, char *filename) {
    char *data = newstr(1);
    strpad(&data, "--");
    strpad(&data, boundary);
    strpad(&data, "\r\n");
    strpad(&data, "Content-Disposition: form-data; name=\"");
    strpad(&data, name);
    strpad(&data, "\"; filename=\"");
    strpad(&data, filename);
    strpad(&data, "\"\r\n");
    strpad(&data, "Content-Type: image/jpeg\r\n\r\n");
    return data;
}

esp_err_t http_post(char *buf, size_t len, char *name, char *filename) {
    esp_err_t ret;
    ...
    esp_http_client_read(client, local_response_buffer, MAX_HTTP_OUTPUT_BUFFER);
